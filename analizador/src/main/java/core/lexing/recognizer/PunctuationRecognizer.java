...
/**
 * Reconocedor de SIGNOS DE PUNTUACIÓN.
 *
 * Responsabilidad:
 *  - Detectar, sin consumir, un signo de puntuación definido en config.json
 ...
 */
public final class PunctuationRecognizer {

    private final OperatorTable table;

    /**
     * @param table tabla construida con el conjunto de puntuación de config.json
     */
    public PunctuationRecognizer(OperatorTable table) {
        this.table = Objects.requireNonNull(table, "OperatorTable no puede ser null");
    }

    /**
     * Intenta reconocer un signo de puntuación en la posición actual.
     *
     * @param cursor flujo de caracteres (no se consume aquí)
     * @return Recognition:
     *   - matched=false si no hay puntuación en esta posición.
